/*
title: '5x5x影院', author: '小可乐 v6.1.1'
ext 鍙€?
{
    "host": "https://www.a3uiz8.com",
    "timeout": 6000,
    "catesSet": "大陆&日韩&欧美&动画&三级&热门&精选",
    "tabsSet": "直连播放&网页播放"
}
    发送任意邮件至此邮箱找回最新域名
5x5xdizhi@gmail.com
...
const DEFAULT_CLASSES = [
    {type_name: '大陆', type_id: 'category/1'},
    {type_name: '日韩', type_id: 'category/2'},
    {type_name: '欧美', type_id: 'category/3'},
    {type_name: '动画', type_id: 'category/4'},
    {type_name: '三级', type_id: 'category/5'},
    {type_name: '热门', type_id: 'tags/%e7%83%ad%e9%97%a8%e9%bb%91%e6%96%99'},
    {type_name: '精选', type_id: 'tags/5x%e7%b2%be%e9%80%89'}
];

async function init(cfg) {
    try {
        HOST = (cfg?.ext?.host?.trim() || 'https://www.a3uiz8.com').replace(/\/$/, '');
        ...
        console.error('鍒濆鍖栧け璐?', e.message);
    }
}
...
async function home(filter) {
    try {
        ...
        console.error('鑾峰彇鍒嗙被澶辫触:', e.message);
...
async function homeVod() {
    try {
        ...
        console.error('鑾峰彇鎺ㄨ崘澶辫触:', e.message);
...
async function category(tid, pg, filter, extend) {
    try {
        ...
        console.error('鑾峰彇鍒嗙被椤靛け璐?', e.message);
...
async function search(wd, quick, pg) {
    try {
        ...
        let searchData = getSearchData(resHtml);
        let VODS = searchData.list.length ? searchData.list : getVodList(resHtml);
        let limit = VODS.length;
        let pagecount = searchData.pagecount || getPageCount(resHtml, pg);

        return JSON.stringify({list: VODS, page: pg, pagecount, limit, total: limit * pagecount});
    } catch (e) {
        console.error('鎼滅储澶辫触:', e.message);
        return JSON.stringify({list: [], page: 1, pagecount: 0, limit: 30, total: 0});
    }
}
...
async function detail(ids) {
    try {
        ...
        if (!resHtml) {throw new Error('婧愮爜涓虹┖');}

        let videoTag = cutStr(resHtml, '<video', '</video>', '', false);
        let playerWrap = getTagById(resHtml, 'player-wrap');
        let titleBlock = resHtml.match(/<div\b[^>]*class\s*=\s*(?:"[^"]*\btext-lg\b[^"]*\bfont-bold\b[^"]*"|'[^']*\btext-lg\b[^']*\bfont-bold\b[^']*'|[^\s>]*\btext-lg\b[^\s>]*)[^>]*>([^]*?)<\/div>/i)?.[1] || '';
        let kname = htmlDecode(getMeta(resHtml, 'og:title') || cleanText(titleBlock) || cutStr(resHtml, '<h4', '</h4>', '名称'));
        let kpic = htmlDecode(getAttr(playerWrap, 'data-poster') || getAttr(videoTag, 'data-poster') || getAttr(videoTag, 'poster') || getMeta(resHtml, 'og:image:secure_url') || getMeta(resHtml, 'og:image'));
        ...
        console.error('璇︽儏澶辫触:', e.message);
        return JSON.stringify({list: []});
    }
}
...
async function play(flag, ids, flags) {
    try {
        ...
        console.error('鎾斁澶辫触:', e.message);
...
function mergeHomeClasses(khtml) {
    ...
    if (khtml) {
        const navReg = /<a\b[^>]*href\s*=\s*(?:"([^"]+)"|'([^']+)'|([^\s>]+))[^>]*>([^]*?)<\/a>/g;
        for (let mt of khtml.matchAll(navReg)) {
            let href = mt[1] || mt[2] || mt[3] || '';
            let typeId = normalizeSitePath(htmlDecode(href)).replace(/^\/|\/$/g, '').replace(/\/page\/\d+$/, '').replace(/\/1$/, '');
            if (!/^(category|tags)\//i.test(typeId) || seen.has(typeId)) {continue;}
            seen.add(typeId);

            let name = cleanText(mt[4]);
            if (!name || /^(首页|视频|小说|搜索|商务|PG|同城|直播)$/i.test(name)) {continue;}
...
function getVodList(khtml) {
    try {
        if (!khtml) {throw new Error('婧愮爜涓虹┖');}
        let kvods = [];
        let seen = new Set();
        let cards = [];

        const a3Reg = /<a\b[^>]*href\s*=\s*(?:"([^"]*\/vd\/\d+\/?)"|'([^']*\/vd\/\d+\/?)'|([^\s>]*\/vd\/\d+\/?))[^>]*\bclass\s*=\s*(?:"[^"]*\bcard\b[^"]*"|'[^']*\bcard\b[^']*'|[^\s>]*\bcard\b[^\s>]*)[^>]*>([^]*?)<\/a>/gi;
        for (let mt of khtml.matchAll(a3Reg)) {
            cards.push(`<a href="${mt[1] || mt[2] || mt[3]}">${mt[4]}</a>`);
        }

        if (!cards.length) {cards = splitCards(khtml, '<div class="video-elem');}

        for (let card of cards) {
            let href = getAttr(card, 'href') || card.match(/href=["'](\/(?:video\/view(?:hd)?|videos\/view|vod\/view(?:hd)?)[^"']*)["']/i)?.[1] || '';
            if (!href) {continue;}
            ...
            let cardTitle = card.match(/<div\b[^>]*class\s*=\s*(?:"[^"]*\bcard-title\b[^"]*"|'[^']*\bcard-title\b[^']*'|[^\s>]*\bcard-title\b[^\s>]*)[^>]*>([^]*?)<\/div>/i)?.[1] || '';
            let kname = cleanText(cardTitle || titleHtml || getAttr(card, 'title') || getAttr(card, 'alt') || '名称');
        ...
        console.error('鐢熸垚瑙嗛鍒楄〃澶辫触:', e.message);
        return [];
    }
}

function getSearchData(khtml) {
    try {
        let json = JSON.parse(khtml || '{}');
        let data = json?.data || {};
        let list = Array.isArray(data.list) ? data.list : [];
        return {
            list: list.map(it => ({
                vod_name: cleanText(it.title || it.name || '名称'),
                vod_pic: absUrl(it.litpic || it.pic || it.cover || ''),
                vod_remarks: '',
                vod_id: absUrl(`vd/${it.id}/`)
            })).filter(it => it.vod_id && !/\/vd\/undefined\//.test(it.vod_id)),
            pagecount: Number(data.total_pages) || Number(data.pagecount) || 0
        };
    } catch (e) {
...
function buildPageUrl(typeId, pg) {
    ...
    if (!path) {path = 'category/1';}

    if (/^(?:category|tags)\//i.test(path)) {
        path = path.replace(/\/page\/\d+$/, '').replace(/\/$/, '');
...
function buildSearchUrl(wd, pg) {
    return absUrl(`api/search/video?keyword=${encodeURIComponent(wd || '')}&page=${pg}`);
}
...
function getAttr(html, name) {
    try {
        let reg = new RegExp(`\\b${escReg(name)}\\s*=\\s*(?:"([^"]*)"|'([^']*)'|([^\\s>]+))`, 'i');
        let mt = String(html || '').match(reg);
        if (!mt) {return '';}
...
function getMetaByKey(khtml, key, value) {
    try {
        let tags = String(khtml || '').match(/<meta\b[^>]*>/gi) || [];
        for (let tag of tags) {
            if (getAttr(tag, key).toLowerCase() === String(value).toLowerCase()) {
                return htmlDecode(getAttr(tag, 'content'));
            }
        }
        return '';
    } catch (e) {
...
function getPlayUrl(khtml) {
    try {
        let playerWrap = getTagById(khtml, 'player-wrap');
        let dataM3u8 = getAttr(playerWrap, 'data-m3u8');
        if (dataM3u8) {return htmlDecode(dataM3u8);}

        let dataMp4 = getAttr(playerWrap, 'data-mp4');
        let dlBase = getAttr(playerWrap, 'data-dl-base');
        if (dataMp4) {return htmlDecode(absUrl(dlBase ? `${dlBase.replace(/\/?$/, '/')}${dataMp4}` : dataMp4));}
...
function getTagById(khtml, id) {
    try {
        let reg = new RegExp(`<[^>]+\\bid\\s*=\\s*(?:"${escReg(id)}"|'${escReg(id)}'|${escReg(id)})(?:\\s|>)[^>]*>`, 'i');
        return khtml.match(reg)?.[0] || '';
    } catch (e) {
        return '';
    }
}
...
function getPageCount(khtml, curPg = 1) {
    try {
        ...
        const regs = [
            /href\s*=\s*(?:"[^"]*\/(?:category|tags)\/[^"']+?\/page\/(\d+)\/?[^"]*"|'[^']*\/(?:category|tags)\/[^']+?\/page\/(\d+)\/?[^']*'|[^\s>]*\/(?:category|tags)\/[^\s>]+?\/page\/(\d+)\/?[^\s>]*)/g,
...
function ctSet(kArr, setStr) {
    try {
        ...
        console.error('ctSet 鎵ц寮傚父:', e.message);
        return kArr;
    }
}
...
function cutStr(str, prefix = '', suffix = '', defVal = '', clean = true, i = 0, all = false) {
    try {
        if (typeof str !== 'string') {throw new Error('cutStr target must be string');}
        ...
        if (isNaN(idx)) {throw new Error('cutStr index must be integer');}
        ...
        console.error('瀛楃涓叉埅鍙栭敊璇?', e.message);
...
async function request(reqUrl, options = {}) {
    try {
        if (typeof reqUrl !== 'string' || !reqUrl.trim()) {throw new Error('reqUrl 涓嶈兘涓虹┖');}
        if (typeof options !== 'object' || Array.isArray(options) || options === null) {throw new Error('options 绫诲瀷閿欒');}
        ...
        console.error(`${reqUrl} -> 璇锋眰澶辫触:`, e.message);
